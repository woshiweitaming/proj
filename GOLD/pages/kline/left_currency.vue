<template>
	<view class="left_currency_class">
		...
		<view class="left_currency_list">
			<view class="labels" v-for="(items, index) in getWSData" :key="index" @tap="change(items.name, items.id, index)">
				<view class="items">
					<view class="label currency">{{items.name}}<text class="market">/USDT</text></view>
...
<script>
	...
	export default {
		...
		props: ['active'],
		methods: {
			change(currency,id,index){
				this.$storage.setLocalData('currentCurrency', index)
				this.$emit('callBack')
				uni.redirectTo({
					url: '/pages/kline/index?currency='+currency+'&id='+id
				})
			}
		},
		computed:{
