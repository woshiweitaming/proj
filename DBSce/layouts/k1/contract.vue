	...
	<view class="contract_class">
		...
		<view class="scroll_bar">
			...
			<view class="record_bar" v-if="active == 0">
				...
				<ContractCommissionRecord :hidden="hidden" v-if="contract.active === 0"  />
				<ContractHistoryRecord :hidden="hidden" v-if="contract.active === 1"  />
			</view>
			<view class="record_main_bar" v-else>
				<ContractPositionRecord :hidden="hidden" />
			</view>
		</view>
		<!-- <EntrustOrder /> -->
	</view>
...
<script>
	...
	import PriceList from '@/layouts/trade1/price_list.vue'
	import TradeMain from '@/layouts/trade1/trade_main.vue'
