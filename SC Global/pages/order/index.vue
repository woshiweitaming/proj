	...
	<view :class="['main_class', createOSClass]">
		<view class="table_class">
			...
			<view :class="['table_main', createOSClass]">
				...
				<view class="content" v-if="menuIndex === 2">
					<view class="record_menu contract_menu">
						<view @tap="changeCoinType(index)" :class="['label', index === coin.active ? 'on' : '']" v-for="(items, index) in coin.menuList" :key="index">{{getLangs(items)}}</view>
					</view>
					<CoinPositionRecord :keys="getKey" v-if="coin.active === 0" />
					<CoinHistoryRecord v-if="coin.active === 1"  />
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapMutations({
				setTimerStatus2: 'SET_TIMER_STATUS2'
			})
		},
		onLoad() {
		...
		onShow(){
			...
			this.futures.active = 0
			this.setTimerStatus2(true)
		},
		onHide(){
			this.setTimerStatus2(false)
		}
	}
</script>
