<template>
	<view class="trade_main_class">
		...
		<view class="trade_fee">
			<text class="trade_fee_label">{{getLangs('fee')}}: {{getFee.toFixed(2)}} USDT</text>
			<text class="trade_fee_label price">{{getLangs('deposit')}}: {{getDepoit.toFixed(2)}} USDT</text>
	...
	export default {
		...
		computed: {
			...
			getDepoit(){
				...
				if(this.getCurrenctData && this.getCurrenctData.close){
					if(typeof this.getCurrenctData.close != 'undefined'){
					    // return (this.form.num * this.getCurrentCurrencyData.close + this.getFee)
						if(this.active == 0){
							return this.getCurrenctData.close / this.form.unit * this.form.num
						}else{
			...
			//手续费
			getFee(){
				if(this.getCurrenctData){
					if(typeof this.getCurrenctData.close != 'undefined'){
						// return this.form.num  * this.getCurrentCurrencyData.close * 0.003
						if(this.active == 0){
							return this.getCurrenctData.close / this.form.unit * this.form.num * 0.03
						}else{
