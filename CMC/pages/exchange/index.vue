...
<script>
	...
	export default {
		...
		data(){
			return {
				account: {
					accountList: ['coin', 'futures', 'contract', 'poolp6'],
					active: 0,
					show: false
				},
				currency_out: {
		...
		computed: {
			...
			getBalance(){
				if(this.getRate && this.getRate.balance){
					...
					const currentAccountData = leftmoney.filter(res => res.type === this.getActive)[0]
			...
			getActive(){
				...
				if(this.account.active === 1){
					return 0
				}
				if(this.account.active === 2){
					return 1
				}
