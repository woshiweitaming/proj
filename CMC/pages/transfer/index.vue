	...
	<view class="transfer_class" @tap="eventClick">
		...
		<view class="account_type">
			...
			<view class="select" @tap.stop="stopClick">
				<view class="select_label" @tap="openDrop">
					<view class="select_name">{{getCurrentAccount}}</view>
				...
				<view class="select_drop" v-show="currency.show">
					<view class="selelct_drop_main" @tap.stop="stopClick">
						<view @tap="changeDrop(key)" class="select_drop_label" v-for="(items, key) in getAccountList"  :key="key" v-show="items != 'type'">{{items}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="num_class">
			...
			<view class="exchange_input">
				<view class="currency">{{getCurrentAccount}}</view>
...
<script>
	...
	export default {
		...
		data(){
			return {
				toData: {
					list: ['coin', 'futures', 'contract', 'poolp1'],
					active: 'futures',
				},
				num: '',
				pList: []
		...
		methods: {
			...
			async balanceTransferHandler(){
				...
				const _in = this.map[this.toData.active]
				const _out = this.map[this.fromData.active]
				const type = this.pList.filter(res => res.name === this.getCurrentAccount)[0].id
				const params = {
					in: _in,
					out: _out,
					num: this.num,
		...
		computed: {
			getBalance1(){
				const type = this.map[this.fromData.active]
				if(this.getAccountBanlance.length == 0) return []
				return this.getAccountBanlance[type][this.getCurrentAccount].usable
			}
		},
		onShow() {
