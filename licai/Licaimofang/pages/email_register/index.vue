<template>
	<view class="register">
		<view class="title">
		  <view class="name">{{getLang('register_p14')}}</view>
		  <view class="link" @click="toPage('/pages/phone_register/index')">{{getLang('register_p1')}}</view>
		</view>
		<view class="form">
			<view class="input_bar">
				<input type="text" v-model="form.email" :placeholder="getLang('register_p15')" class="input" />
			</view>
			<view class="input_bar code">
				<input type="text" v-model="form.vcode" :placeholder="getLang('register_p8')" class="input" />
				<view class="send" @click="getEmailCode">{{getCodeTxt}}</view>
			</view>
			<view class="input_bar">
			...
			<u-radio-group  v-model="checked">
				<u-radio class="radio" label-size="12">{{getLang('register_p3')}}<text class="link">{{getLang('register_p4')}}</text></u-radio>
			</u-radio-group>
			<view class="btn_bar">
