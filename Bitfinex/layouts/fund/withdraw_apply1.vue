	...
	<view class="form_class">
		<view class="withdraw_content">
			...
			<view class="form_label">
				...
				<view class="form_label_main">
					<view class="form_input_bar">
						<input type="text" class="input_bar" @input="inputHandler" :placeholder="getLangs('bankAccountNumber')" v-model="params.banknumber" />
					</view>
				</view>
			</view>
			<view class="form_label">
...
<script>
	...
	export default {
		...
		methods:{
			...
			inputHandler(e){
				const res = getBanks(e.detail.value)
				if(res){
					const index = banks.findIndex(d => d == res.bankName)
					this.banksIndex = index
				}
			}
		},
