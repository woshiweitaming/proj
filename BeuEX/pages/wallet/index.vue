			...
			<view class="labels" v-else v-for="(items, index) in dataList" :key="index">
				<uni-swipe-action>
				    <uni-swipe-action-item :options="options" @click="onClick($event, items)">
				         <view class="items"><text class="currency">{{toType(items.type)}}</text><text class="box">{{items.chainname}}</text></view>
...
<script>
	...
	import { getPayAddList, delPayAddress, getWithdrawalTypeList } from '@/api/user.js'
	export default {
		name: 'Wallet',
		...
		computed: {
			toType(){
				return type => {
					const typeList = this.currecyList
					return typeList.length == 0 ? '' : typeList[type]
				}
			}
		},
		methods: {
