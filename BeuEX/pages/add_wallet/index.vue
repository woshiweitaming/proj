		...
		<view class="linked_main">
			<view class="select" @tap.stop="stopClick">
				<view class="select_label" @tap="currencyStatus = !currencyStatus">
					<view class="select_name">{{currecyList.length > 0 && currecyList[currencyActive]}}</view>
				...
				<view class="select_drop" v-show="currencyStatus">
					<view class="selelct_drop_main" @tap.stop="stopClick">
						<view @tap="changeDrop1(key)" class="select_drop_label" v-for="(items, key) in currecyList"  :key="key">{{items}}</view>
					</view>
				</view>
			</view>
		</view>
