	...
	<view class="currency_class">
		<view class="show_data" v-if="getWSData.length > 0">
			<view class="show_label" @tap="toKpage(items.name, items.id)" v-for="(items, index) in getTopData" :key="index">
				<view class="currency"><text class="currency_name">{{items.name}}</text></view>
		...
		<view class="currency_main">
			...
			<view class="currency_label" v-for="(items, index) in getWSSortData && getWSSortData" :key="index" @tap="toKpage(items.name, items.id)">
				<view class="currency_bd_label head">
					<view :class="['currency','circle', `circle${index}`]">{{items.name2}}</view>
					<text class="volume">{{items.name}}</text>
				</view>
...
<style scoped>
	...
	.circle.circle1{
		background: #1D8BEE!important;
	}
	.circle.circle2{
		background: #3E39A8!important;
	}
	.circle.circle3{
		background: #669900!important;
	}
	.circle.circle4{
		background: #f04874!important;
	}
	.circle.circle5{
		background: #f06c48!important;
	}
	.circle.circle6{
		background: #ebc52b!important;
	}
	.circle.circle7{
		background: #2beb98!important;
	}
	.circle.circle8{
		background: #462beb!important;
	}
	.circle.circle9{
		background: #2baaeb!important;
	}
	.circle.circle10{
		background: #eb2bbc!important;
	}
	.circle.circle11{
		background: #3c2f80!important;
	}
	.circle.circle12{
		background: #91df3d!important;
	}
	.circle.circle13{
		background: #801094!important;
	}
	.spacer{
		background: #f9f9f9;
		height: 20upx;
