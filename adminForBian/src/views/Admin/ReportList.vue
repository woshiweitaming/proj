            ...
            <div class="table_class">
                <div class="level_list">
                    <span class="labels" v-for="(items, index) in userList" :key="index">
                        ...
                        <b @click="queryData(index, items.uid)">{{items.uname}}</b>
                    </span>
                </div>
                <div class="table_class">
                    <el-table
                    ...
                    >
                        ...
                        >
                           <template slot-scope="scope">
                               <el-button size="mini" v-if="curLevel != 3" @click="queryNext(scope.row)" type="primary">下级</el-button>
                           </template>
                        </el-table-column>
                    </el-table>
                </div>
...
export default {
    ...
    methods: {
        ...
        queryNext(data, type){
            ...
            const uid = data.uid
            this.curLevel++
            this.userList.push({uname: uname, uid: uid})
            this.params.type = this.curLevel
            this.params.parentID = uid
            this.search()
        },
        queryData(index, uid){
            ...
            this.params.parentID = index === 0 ? 0 : uid
            this.search()
        },
        geKS(ck, tk){
    ...
    created(){
        ...
        this.search()
        const user = {uname: this.$storage.getLocalData('uname'), uid: this.$storage.getLocalData('uid')}
