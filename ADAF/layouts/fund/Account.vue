<template>
	<view class="account_class">
		...
		<view class="account_list">
			<view class="account_list_label" v-for="(items, key) in accountList" :key="key">
				<view v-if="key !== 'type'">
					...
					<view class="table_hd">
						...
						<view class="table_hd_label">{{getLangs('convert')}}(USDT)</view>
					</view>
					<view class="table_bd">
...
<script>
	...
	export default {
		...
		data(){
			return {
				menuList: ['coin', 'contract', 'futures'],
				active: 0
			}
		},
		methods: {
		...
		computed: {
			...
			accountList(){
				if(this.getAccountBanlance.length){
					const data = this.getAccountBanlance[this.getActive]
					let obj = {}
					for(let key in data){
						if(key === 'BTC' || key === 'USDT' || key === 'ETH' || key === 'GEC'){
							obj[key] = data[key]
						}
					}
					console.log(obj)
