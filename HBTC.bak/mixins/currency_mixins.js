...
const currencyMixins = {
	...
	computed: {
		...
		getCurrentCurrencyData(){
			let id = this.$storage.getLocalData('currentCurrency')
			const pages = getCurrentPages()
			const page = pages[pages.length - 1]
			const curParam = page.options || page.$route.query
			const curId = curParam.id
			if(typeof curId !== 'undefined'){
				id = curId
			}
			console.log(curParam)
			return id == '' || id == null ? this.getWSData[0] : this.getWSData.filter(res => res.id == id)[0]
		},
		...mapGetters({
