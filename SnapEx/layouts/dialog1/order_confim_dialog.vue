				...
				<view class="count_down">
					...
					<view v-if="countDown > 0">
						...
						<view class="info">
							...
							<text :class="['price', getCurrentCurrencyData.gain > 0 ? 'green' : 'red' ]">{{getCurrentCurrencyData && getCurrentCurrencyData.close}}</text>
						</view>
					</view>
					<view v-else>
	...
	export default {
		...
		computed: {
			...
			getExpectedEarnings(){
				...
				//当前价格
				const nowPrice = Number(this.getCurrentCurrencyData.close)
				//执行价
