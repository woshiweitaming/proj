			...
			<view class="pop_main">
				<view class="count_down">
					...
					<view v-if="countDown > 0">
						...
						<view class="info">
							...
							<text :class="['price', getCurrenctData.gain > 0 ? 'green' : 'red' ]">{{getCurrenctData.close}}</text>
						</view>
					</view>
					<view v-else>
				...
				<view class="trade_order_info">
					<view class="label">
						...
						<view :class="['value',getTradeParams.type == 0 ? 'green' : 'red']">{{getTradeParams.type == 0 ? getLangs('buy') : getLangs('sell')}}</view>
					</view>
					<view class="label">
	...
	export default {
		...
		computed: {
			...
			getExpectedEarnings(){
				...
				//当前价格
				const nowPrice = Number(this.getCurrenctData.close)
				//执行价
