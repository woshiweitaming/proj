		...
		<view class="withdraw_content">
				...
				<view class="form_label_main">
					<view class="form_input_bar">
						<view class="select" @tap.stop="stopClick">
							<view class="select_label" @tap="show1 = !show1">
								<view class="select_name">{{currencyList[curActive].name}}</view>
								<text class="iconfont icon-unfold"></text>
							</view>
							<view class="select_drop" v-show="show1">
								<view class="selelct_drop_main" @tap.stop="stopClick">
									<view @tap="changeDrop1(key)" class="select_drop_label" v-for="(items, key) in currencyList"  :key="key">{{items.name}}</view>
								</view>
							</view>
						</view>
					</view>
			...
			<view class="form_label">
				...
				<view class="form_label_main">
					...
					<text class="sub">{{getLangs('avaliableUsdt')}}({{currencyList[curActive].name}}) : {{config.usdt}}</text>
				</view>
			</view>
			<view class="form_label">
...
<script>
	...
	export default {
		...
		methods:{
			...
			async init(){
				const res = await getWinConfig({type: this.curActive})
		...
		computed: {
			...
			getWalletList(){
				...
				const arr = this.walletList.filter(res => res.type == this.curActive)
