			...
			<view class="order_record record_main common" v-if="type == 1">
				...
				<scroll-view class="scroll" :scroll-y="true" v-else>
					<view class="record_label" v-for="(items, index) in dataList" :key="index">
						<view class="label_title">
							<view class="label_name">
								 {{getLangs('withdrawBtn')}}
								 <text :class="['status','deal'+items.status]">{{toStatus(items.status)}}</text>
							</view>
						</view>
						<view class="labels_main_normal withdraw">
							<view class="labels_box">
								<view class="text">{{getLangs('orderno')}}</view>
							...
							<view class="labels_box">
								<view class="text">{{getLangs('bankname')}}</view>
								<view class="value">{{items.bankname}}</view>
							</view>
							<view class="labels_box">
								<view class="text">{{getLangs('banknumber')}}</view>
								<view class="value">{{items.banknumber}}</view>
							</view>
							<view class="labels_box">
								<view class="text">{{getLangs('customerName')}}</view>
								<view class="value">{{items.zhenname}}</view>
							</view>
						</view>
						<view class="labels_fd" v-if="items.remarks">
							<view class="labels">{{getLangs('remark')}}:{{items.remarks}}</view>
...
<script>
	...
	import { getPay, revokePay, getWithdrawalBank } from '@/api/record.js'
	export default {
		name: 'Recharging',
...
<style scoped>
	...
	.withdraw .labels_box:last-child{
		border-bottom: none;
	}
	.withdraw .value{
		color: #333 !important;
