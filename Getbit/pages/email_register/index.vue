<template>
	<view class="login">
		<view class="login_title">邮箱注册</view>
		<view class="tips">使用常用邮箱进行注册，更便捷</view>
		<view class="login_form">
			...
			<view class="text">注册代表同意<text class="link"> 《本平台用户协议》</text></view>
			<view class="text link"  @click="toPage('/pages/phone_register/index')">手机注册</text></view>
			<view class="login_input_bar">
				<view :class="['buttons', getActive]" @click.native.stop="handler">下一步</view>
			</view>
			<view class="tips">
				<view class="block">已有账户? <text class="tips_text" @click="toPage('/pages/login_email/index')">登录</text></view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async getEmailCodeHandler(){
				...
				// }
				uni.navigateTo({
					url: '/pages/email_captcha/index?qh='+params.area+'&imgcode='+params.imgcode+'&email='+params.to
				})
			},
			toWebPage(){
				uni.navigateTo({
