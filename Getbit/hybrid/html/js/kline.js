...
var app = new Vue({
	...
	methods:{
		getData(status){
			...
			axios.get(url,{
				...
				const datas = data.map(d => {
					return [that.format(parseInt(d.id)), d.open, d.close, d.high, d.low, d.amount]
				})
				that.datas = datas.map(d => {
					return [d[1],d[2],d[3],d[4],d[5]]
				})
				that.dates = datas.map(function(item) {
	                return item[0]
                })
				that.volumes = datas.map(function(item, index) {
		...
		// 获取k线数据,生成k线
		getKline(){
			const dataMA5 = this.calculateMA(5, this.datas);
			const dataMA10 = this.calculateMA(10, this.datas);
			const dataMA30 = this.calculateMA(30, this.datas);
			const volumeMA5 = this.calculateMA(5, this.volumes);
			const volumeMA10 = this.calculateMA(10, this.volumes);
			const macd = this.calcMACD(12,26,9,this.datas,1);
			const ma5 = this.MA(5,this.datas)
			const ma10 = this.MA(10,this.datas)
			myChart.setOption({
				xAxis: [
					{
						data: this.dates
		...
		draw(){
			...
			var option = {
				...
				xAxis: [{
					...
					data: that.dates, //类目数据，在类目轴（type: 'category'）中有效。
					scale: true,
