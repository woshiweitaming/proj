<template>
	<view class="trade_main_class" @tap="eventClick">
		...
		<view class="trade_fee">
			<view class="trade_fee_label">{{getLangs('fee')}}: {{getFee.toFixed(2)}} USDT</view>
			<view class="trade_fee_label price">{{getLangs('deposit')}}: ≈ {{getDepoit.toFixed(2)}} USDT</view>
	...
	export default {
		...
		data(){
			return {
				...
				dropshow: false,
				dropshow1: false,
				dropList: [{name: '1X',value: 1}, {name: '2X',value: 2},{name: '5X',value: 5},{name: '10X',value: 10},{name: '50X', value: 50},{name: '100X',value: 100}],
				//记录
				contract: {
		...
		computed: {
			//计算价格
			getPrice(){
				return (this.form.num  * this.getCurrentCurrencyData.close + Number(this.getFee)).toFixed(2)
			},
			//计算价格
			getDepoit(){
				// return (this.num * this.getUnit * this.getWSData.close * this.getRatio).toFixed(2)
				if(this.getCurrentCurrencyData && this.getCurrentCurrencyData.close){
					if(typeof this.getCurrentCurrencyData.close != 'undefined'){
					    // return (this.form.num * this.getCurrentCurrencyData.close + this.getFee)
						if(this.active == 0){
							return this.getCurrentCurrencyData.close / this.form.unit * this.form.num
						}else{
							return this.form.appointprice / this.form.unit * this.form.num
						}
					}
				}
				return 0
			...
			getFee(){
				...
				// }
				if(this.getCurrentCurrencyData){
					if(typeof this.getCurrentCurrencyData.close != 'undefined'){
						// return this.form.num  * this.getCurrentCurrencyData.close * 0.003
						if(this.active == 0){
							return this.getCurrentCurrencyData.close / this.form.unit * this.form.num * 0.03
						}else{
							return this.form.appointprice / this.form.unit * this.form.num * 0.03
						}
					}
				}
				return 0
