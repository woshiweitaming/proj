	...
	<view class="futures_trade_class" @tap="eventClick">
		<view class="futures_section">
			<view class="futures_currency">
			   {{currency}}
			   <text @tap="show = true" class="iconfont icon-other"></text>
		...
		<view class="futures_section">
			<view class="futures_record">
				<view class="futures_title">
				    <view @tap="changeMenu(index)" :class="['menu_label', index == mactive ? 'on' : '']" v-for="(items, index) in menuList1" :key="index">{{getLangs(items)}}</view>
					<view @tap="toPage" class="white_paper_icon">{{getLangs('introduce')}}</view>
				</view>
				<view class="record_content">
					<coinPositionRecord v-if="mactive === 0" />
					<CoinHistoryRecord v-if="mactive === 1" />
				</view>
			</view>
		</view>
	...
	export default {
		...
		data(){
			return {
				...
				menuList1: ['commissionRecord', 'history'],
				mactive: 0,
				show: false,
				key: ''
			}
		...
		methods: {
			...
			async trade(){
				const id = this.id
				const num = this.per === '' ? this.form.num : this.getNum
...
<style scoped>
	...
	.futures_title .menu_label{
		padding: 0 10upx;
		display: inline-block;
		line-height: 60upx;
		margin-right: 20upx;
	}
	.futures_title .menu_label.on{
		background: #007AFF;
		color: #fff;
		border-radius: 5px;
	}
