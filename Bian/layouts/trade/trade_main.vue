<template>
	<view class="trade_main_class" @tap="eventClick">
		...
		<view class="trade_fee">
			<view class="trade_fee_label">{{getLangs('fee')}}: {{getFee.toFixed(2)}}</view>
			<view class="trade_fee_label price">{{getLangs('deposit')}}: ≈ {{getPrice}}</view>
	...
	export default {
		...
		computed: {
			...
			getCurrentCurrencyData(){
				if(this.getWSData.length){
					const data = this.getWSData.filter(res => res.id == this.id)[0]
					return data
				}
				return {}
			},
