<template>
	<view class="k_line_data">
		<view class="k_line_card">
			<view @tap="sliderCurrency" class="currency">{{getCurrentCurrencyData && getCurrentCurrencyData.name}} / USDT <text class="iconfont icon-other"></text></view>
			<view class="position" v-if="getCurrentTradeType === 0">
				<image class="kicon" mode="widthFix" src="../../static/images/k.png" @tap="navigateTo('/pages/kline/index?currency='+getCurrentCurrencyData.name+'&id='+getCurrentCurrencyData.id)"></image>
			</view>
			<!-- <view class="k_data">
				<view class="labels">
					...
					<view :class="['live_label', getClass(getCurrentCurrencyData && getCurrentCurrencyData.gain)]">{{getCurrentCurrencyData && getCurrentCurrencyData.gain}}%</view>
				</view>
				<view class="labels">
					<view class="inner">
						<view class="inner_label">
							<text class="name">{{getLangs('openPrice')}}</text>
							<text class="value">{{getCurrentCurrencyData && getCurrentCurrencyData.open}}</text>
						</view>
						<view class="inner_label">
							<text class="name">{{getLangs('turnover24h')}}</text>
							<text class="value">{{((getCurrentCurrencyData && Number(getCurrentCurrencyData.vol))/10000).toFixed(4)}}W</text>
						</view>
						<view class="inner_label">
							<text class="name">{{getLangs('lowestPrice')}}</text>
							<text class="value">{{getCurrentCurrencyData && getCurrentCurrencyData.low}}</text>
						</view>
						<view class="inner_label">
							<text class="name">{{getLangs('highestPrice')}}</text>
							<text class="value">{{getCurrentCurrencyData && getCurrentCurrencyData.high}}</text>
						</view>
					</view>
				</view>
			</view> -->
...
<style scoped>
	...
	.k_data .labels:first-child{
		width: 45%;
	}
	.k_data .labels:last-child{
		width:55%;
	}
