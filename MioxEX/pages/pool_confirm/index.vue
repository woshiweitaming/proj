	...
	<view class="page">
		<view class="pool_redeem_confirm">
			<view class="logo">
				...
				<view class="name">{{data.pname}}</view>
			</view>
			<view class="pool_hd">
				...
				<view class="pool_label">
					...
					<view class="name">{{getLangs('estimatedEarnings')}}({{data.pname}})</view>
				</view>
			</view>
			<view class="pool_bd">
				<view class="pool_label">
				...
				<view class="pool_label">
					...
					<view class="value">{{data.beginDate}}</view>
				</view>
				<view class="pool_label">
				...
				<view class="pool_label">
					...
					<view class="value">{{data.createDate}}</view>
				</view>
			</view>
			<view class="bottom_button">
				<view @tap="submit" class="btn">{{getLangs('poolp27')}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	import { payMiningPoolOrder, submitMiningPoolOrder } from '@/api/user.js'
	...
	export default {
		...
		methods: {
			// async getDetail(){
			// 	const res = await submitMiningPoolOrder(this.params)
			// 	this.data = res.data
			// },
			async submit(){
				const params = {orderNo: this.data.orderNo}
				const res = await payMiningPoolOrder(params)
				this.backTips(res)
				if(validate.checkCode(res)){
					this.navigateTo('/pages/pool_success/index?type=1')
				}
			}
		},
		onLoad(opt){
			this.params.mid = opt.mid
			this.params.amount = opt.amount
			this.params.period = opt.period
			this.data = opt
