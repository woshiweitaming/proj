<template>
	<view class="trade_main_class">
		...
		<view class="trade_form">
			...
			<view class="trade_form_label">
				<view class="trade_inner">
					...
					<input type="text" :disabled="true" v-if="active === 0" class="input_bar" :placeholder="getLangs('entrustEmptyTips')" v-model="getCurrenctData.close"/>
		...
		<view class="trade_fee">
			<text class="trade_fee_label">{{getLangs('fee')}}: {{getCurrenctData.attach && getCurrenctData.attach.fee}}</text>
			<text class="trade_fee_label price">{{getLangs('deposit')}}: {{getDepoit}}</text>
			<!-- <text class="trade_fee_label price">{{getPrice}}</text> -->
		</view>
		<view @tap="trade" :class="['trade_btn', getClass]">{{action == 0 ? getLangs('buy') : getLangs('sell')}}</view>
